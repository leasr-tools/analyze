...
def parse_summary_data(text):
    ...
    # Purchase Price
    price_match = re.search(r"Purchase\s+Price\s*\$?(\d{1,3}(?:,\d{3})*)", text, re.IGNORECASE)
    if price_match:
        data["purchase_price"] = float(price_match.group(1).replace(",", ""))

    # Operating Expenses
    opex_match = re.search(r"Operating\s+Expenses\s*\$?(\d+\.?\d*)", text, re.IGNORECASE)
    if opex_match:
        data["opex_psf"] = float(opex_match.group(1))
...
def analyze_scenario(purchase_price, monthly_expenses, rent_psf, square_feet,
    ...
    loan_balance_end = schedule.loc[hold_period * 12 - 1, "Balance"] if hold_period * 12 <= len(schedule) else 0

    # Annual Cash Flow
    monthly_debt_service = schedule.loc[:hold_period * 12 - 1, "Payment"].mean()
...
if comp_input == "Upload CoStar/Title Report":
    ...
    if uploaded_file:
        with st.spinner("Extracting PDF data..."):
            ...
            pdf_data.update({k: v for k, v in parse_summary_data(text).items() if v})
        ...
        - **Operating Expenses (if found):** ${pdf_data['opex_psf']:.2f}/sqft/year
        - **Square Footage:** {pdf_data['square_feet']:,} sqft
        - **Purchase Price (if found):** ${pdf_data['purchase_price']:,}
        """)
else:
    ...
    pdf_data["opex_psf"] = st.number_input("Operating Expenses ($/sqft/year)", value=3.0)
    pdf_data["square_feet"] = st.number_input("Square Feet", value=10000)
    pdf_data["purchase_price"] = st.number_input("Purchase Price ($)", value=1000000)

# Step 2: General Parameters
st.markdown("## Step 2: General Parameters")
col_gen1, col_gen2, col_gen3 = st.columns(3)
with col_gen1:
    purchase_price = st.number_input("Purchase Price ($)", value=pdf_data['purchase_price'])
    monthly_expenses = st.number_input("Monthly Operating Expenses ($)", value=pdf_data['opex_psf'] * pdf_data['square_feet'] / 12)
with col_gen2:
...
if st.button("Analyze Deal"):
    general = {
        "purchase_price": purchase_price, "monthly_expenses": monthly_expenses,
