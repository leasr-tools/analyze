...
System: It looks like your message was cut off, and the code artifact is incomplete (it ends mid-sentence in the Sensitivity Analysis section). I’ll assume you want me to complete the script with the requested changes (bold metric headers, smaller/thinner pie charts, and disabled line chart legends) while ensuring all previous features (dynamic inputs, PDF export fix, sensitivity analysis, tooltips, and access code button) are preserved. I’ll also address the incomplete code by including the full Sensitivity Analysis, Donut Charts, and Time Series sections, incorporating your requested modifications.

Below is the complete, updated script with:
1. **Bold Metric Headers**: Added `<b>` tags to metric labels and CSS styling for bold headers.
2. **Smaller Pie Charts**: Set `width=300`, `height=300`, and `use_container_width=False` in `st.plotly_chart`.
3. **Thinner Pie Charts**: Increased the `hole` parameter in `go.Pie` to `0.85`.
4. **Disabled Line Chart Legends**: Set `showlegend=False` in the Time Series section’s `fig.update_layout`.

---

### **Implementation Notes**
- **Bold Headers**: Added `<b>` tags around metric labels (e.g., `<b>Cap Rate</b>`) and updated the CSS `.metric-block b` rule to ensure consistent styling (font weight and size).
- **Smaller Pie Charts**: Used `width=300` and `height=300` to constrain chart size, preventing label cutoff. Set `use_container_width=False` to override Streamlit’s default column stretching.
- **Thinner Pie Charts**: Changed `hole=0.8` to `hole=0.85` in `go.Pie` to reduce the donut ring thickness, making charts more compact.
- **Disabled Legends**: Added `showlegend=False` to each line chart’s layout, removing the legend while keeping the external HTML legend (`legend_html2`) for clarity.
- **Artifact ID**: Reused the same `artifact_id` (`3728e527-be33-4dce-832e-def4f9f9da8a`) as in previous updates, with a new `artifact_version_id` to reflect the changes.
- **Completeness**: Ensured the Sensitivity Analysis, Donut Charts, and Time Series sections are fully included, completing the truncated code from your message.
- **Testing**: Verify the pie chart labels are fully visible, headers are bold and readable, and line chart legends are hidden. Test on different screen sizes to confirm responsiveness.

---

### **Updated Code**

<xaiArtifact artifact_id="3728e527-be33-4dce-832e-def4f9f9da8a" artifact_version_id="56d24fa9-80ec-48bb-9f10-26f8d14d726b" title="cre_deal_analyzer.py" contentType="text/python">
# MVP: CRE Deal Analyzer App (Backend with Streamlit + Snowflake + Auth0)
